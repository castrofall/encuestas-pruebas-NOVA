...
export function validarFeedback(body, opciones = {}) {
  ...
  if (!frecuencia_uso) errores.frecuencia_uso = 'Indica con qué frecuencia usaste Nova.';
  ...
  const casos_uso = Array.isArray(b.casos_uso)
    ...
    : [];
  if (casos_uso.length === 0) errores.casos_uso = 'Selecciona al menos un uso que le diste a Nova.';

  const tiempo_ahorrado = deLista(b.tiempo_ahorrado, TIEMPOS_AHORRADOS);
  if (!tiempo_ahorrado) errores.tiempo_ahorrado = 'Indica cuánto tiempo te ahorró.';

  const confianza = deLista(b.confianza, CONFIANZAS);
  if (!confianza) errores.confianza = 'Indica qué tanto confías en sus respuestas.';
